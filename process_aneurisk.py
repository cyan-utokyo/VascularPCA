...
aneurisk_dir = mkdir("./", "aneurisk")
aneurisk_raw_dir = mkdir(aneurisk_dir, "raw")

pre_files = glob.glob("D:/!_Aneurisk/cma/*.vtk")
pre_unaligned_curves = []

for idx in range(len(pre_files)):
    filename = pre_files[idx].split("\\")[-1].split(".")[0]
    print (filename)
    pt, Curv, Tors, Radius, Abscissas, ptns, ftangent, fnormal, fbinormal = GetMyVtk(pre_files[idx], frenet=1)
    pre_unaligned_curves.append(pt)
vm_dir = mkdir(aneurisk_dir, "vm")
vm_dir = mkdir(vm_dir, "txt")

# print ("使用できるデータ：", len(Files))
Files = []
unaligned_curves = []
for i in range(len(pre_files)):
    filename = pre_files[i].split("\\")[-1].split(".")[0]
    curve_length = measure_length(pre_unaligned_curves[i])
    if curve_length > 64:
        f=open(vm_dir+filename+".txt","w+")
        f.write("#pt3d {}\n".format(len(pre_unaligned_curves[i])))
        Files.append(pre_files[i])
        scaled_curve = pre_unaligned_curves[i] * (64/curve_length)
        unaligned_curves.append(scaled_curve[::-1])
        makeVtkFile(aneurisk_raw_dir+filename+".vtk", scaled_curve[::-1], [], [])
        print (aneurisk_raw_dir+filename+".vtk")
        for k in range(len(scaled_curve[::-1])):
            f.write("{} {} {}\n".format(scaled_curve[::-1][k][0], scaled_curve[::-1][k][1], scaled_curve[::-1][k][2]))

print (len(Files), len(unaligned_curves))

import os

vtk_file_paths = glob.glob("D:\\!BraVa_src\\src\\ModeDecomposition\\aneurisk\\raw\\*.vtk")
output_dir = 'D:\\!BraVa_src\\src\\ModeDecomposition\\aneurisk\\resampling\\'

for file_path in vtk_file_paths:
    # 获取文件的基本名，例如"C0098.vtk"
    basename = os.path.basename(file_path)
    
    # 构建输出文件路径
    output_file_path = os.path.join(output_dir, basename)

    # 构建vmtkcenterlineresampling命令
    command = f'vmtkcenterlineresampling -ifiles {file_path} -length 1.0 -ofile {output_file_path}'

    # 输出命令
    # print(command)
    # os.system("vmtk "+command)

pre_files = glob.glob("D:\\!BraVa_src\\src\\ModeDecomposition\\aneurisk\\resampling\\*.vtk")
unaligned_curves = []
...
print (unaligned_curves.shape)
print (unaligned_curves.shape)
a_curves = align_icp(unaligned_curves,base_id=1)
print ("First alignment done.")
Procrustes_curves = align_procrustes(a_curves,base_id=-1)
print ("procrustes alignment done.")


print (len(a_curves))
print (len(unaligned_curves))
print (len(Files))
np.save("D:\\!BraVa_src\\src\\ModeDecomposition\\aneurisk\\procrustes_curves.npy", Procrustes_curves)
np.save("D:\\!BraVa_src\\src\\ModeDecomposition\\aneurisk\\filename_procrustes_curves.npy", Files)
