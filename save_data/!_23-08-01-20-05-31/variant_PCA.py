...
print ("aneuriskデータ：", len(aneurisk_file))
...
a_aneurisk_curves = align_icp(aneurisk_curves, base_id=-2, external_std_curve=a_curves[base_id])
print ("First alignment done.")
Procrustes_curves = align_procrustes(a_curves,base_id=base_id) # 这里要使用bh0017对齐而不是平均，具体参考geodesic_optimize.ipynb结果
...
parameterized_aneurisk_curves = np.zeros_like(Procrustes_aneurisk_curves)
...
for i in range(len(Procrustes_aneurisk_curves)):
    parameterized_aneurisk_curves[i] = arc_length_parametrize(Procrustes_aneurisk_curves[i])


Aligned_curves = align_curve(parametrized_curves)
Procrustes_curves = np.array(Procrustes_curves)
Aligned_anurisk_curves = align_curve(parameterized_aneurisk_curves)
Procrustes_aneurisk_curves = np.array(Procrustes_aneurisk_curves)
...
for i in range(len(Aligned_anurisk_curves)):
    aligned_length = measure_length(Aligned_anurisk_curves[i])
    procrustes_length = measure_length(Procrustes_aneurisk_curves[i])
    Aligned_anurisk_curves[i] = Aligned_anurisk_curves[i] * (procrustes_length/aligned_length)

for i in range(len(Aligned_curves)):
    plt.plot(Aligned_curves[i,:,0],color="k")
    plt.plot(unaligned_curves[i,:,0],color="r")
plt.text(0,10,"PCA aligned is effective",color="blue")
plt.savefig(bkup_dir+"VarianceAligned_VS_NotAligned.png")
plt.close()
...
fig = plt.figure(dpi=300)
ax = fig.add_subplot(111)
ax.hist([Procrustes_curves.flatten(),
         Aligned_curves.flatten(),
         Procrustes_aneurisk_curves.flatten(),
         Aligned_anurisk_curves.flatten()], bins=50,
        label=["Procrustes aligned", "Variance aligned", "Procrustes with Aneurisk", "Variance aligned with Aneurisk"],
        color=[cmap(0.9), cmap(0.7) ,cmap(0.6),cmap(0.2)], )

ax.grid(linestyle=":", alpha=0.4)
ax.set_xlabel("x(mm)")
ax.set_ylabel("Frequency")
plt.legend()
plt.savefig(bkup_dir+"hist_alignment_withAneurisk.png")
plt.close()
...
for loop in range(6):
    ...
    test_procrustes_geodesic_d = compute_geodesic_dist(procrustes_curves[train_num:],curve_A=np.mean(procrustes_curves[:train_num],axis=0))
    test_aligned_geodesic_d = compute_geodesic_dist(aligned_curves[train_num:],curve_A=np.mean(aligned_curves[:train_num],axis=0))
