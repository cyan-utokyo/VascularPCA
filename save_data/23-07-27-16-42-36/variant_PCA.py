...
for loop in range(1):
    ...
    for data_key, data_values in data_dict.items():
        ...
        train_inverse_parts = train_inverse_large.reshape(*train_data.shape)
        print (train_inverse_parts.shape)
        test_inverse_parts = test_inverse_large.reshape(*test_data.shape)
        train_inverse = []
        test_inverse = []
        for i in range(3):
        ...
        train_data_inverse = np.array(train_inverse)
        test_data_inverse = np.array(test_inverse)
