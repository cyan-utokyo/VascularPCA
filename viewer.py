...
def compute_curvature_and_torsion_from_vtk(poly_data):
    # 这里是一个模拟的计算函数
    # 真实的计算会涉及到更复杂的几何分析
    ...
    # 生成模拟数据
    curvature = np.random.random(num_points)
    torsion = np.random.random(num_points)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.sliders = []
        self.slider_labels = ['MoD 1', 'MoD 2', 'MoD 3', 'MoD 4', 'MoD 5', 'MoD 6', 'MoD 7', 'MoD 8']
        ...
        for i in range(8):
            ...
            slider.setValue(5)
            slider.valueChanged.connect(lambda value, idx=i: self.update_sliders(idx, value))
            self.sliders.append(slider)

            # 创建标签
            label = QtWidgets.QLabel(self.slider_labels[i])
            label.setAlignment(QtCore.Qt.AlignCenter)

            # 将滑块和标签添加到网格布局中
            self.left_panel.addWidget(label, i, 0)  # 添加标签
            self.left_panel.addWidget(slider, i, 1)  # 添加滑块

            row = i // 2  # 确定行
            col = i % 2   # 确定列
            self.left_panel.addWidget(slider, row, col)
    ...
    def display_polyline(self, file_name):
        ...
        curvature, torsion = compute_curvature_and_torsion(pts)
        # 使用曲率和挠率数据更新右侧图表
